...
// Removed centralized logAction API calls. Replaced with vendorLogger for direct logging.
...
exports.registerVendor = async (req, res) => {
  ...
  try {
    ...
    vendorLogger.info(`${username} registered successfully.`);
    res.status(201).json({ message: 'Vendor registered successfully' });
  } catch (error) {
    vendorLogger.error(`Error registering vendor: ${error.message}`);
...
exports.vendorLogin = async (req, res) => {
  ...
  try {
    ...
    if (userResult.length === 0) {
      ...
      vendorLogger.warn(`${username} attempted to login but was not found`);
    ...
    if (new Date(user.password_expires) <= now) {
      ...
      vendorLogger.warn(`${username} attempted to login with expired password`);
    ...
    if (!passwordMatch) {
      ...
      vendorLogger.warn(`${username} attempted to login with incorrect password`);
      return res.status(401).json({ error: 'Invalid username or password' });
    }
    ...
    vendorLogger.info(`${username} has logged in successfully`);

    res.status(200).json({ message: 'Login successful', token });
  } catch (error) {
    vendorLogger.error(`Error during vendor login: ${error.message}`);
...
exports.vendorLogout = async (req, res) => {
  ...
  if (!username) {
    vendorLogger.error(`Attempted logout with missing username.`);
  ...
  try {
    ...
    vendorLogger.info(`${username} has logged out successfully`);
    ...
    vendorLogger.error(`Error during vendor logout for user ${username}: ${error.message}`);
